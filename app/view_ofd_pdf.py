...
from src.utils import OCRPaser, ofd_zip_to_json
import numpy as np
import cv2
import tornado
...
__ALLOWED__ = ["jpg", "JPG", "JPEG", "png",
               "PNG", "jpeg", "pdf", "PDF", "ofd", "OFD"]
...
class MyApp:
    ...
    def __init__(self) -> None:
        ...
        # 配置端口号，接口地址，上传文件字段名, ofd解析文件名
        self._fields = {"upload_field": "upload_src",
                        "port": 5000,
                        "restfulapi": "/ocrpaser",
                        "ofd_paser_xml": [
        ...
        self.model = OCRPaser(rec_model_dir=rec_model_path,
                              det_model_dir=det_model_path, cls_model_dir=cls_model_path)
    ...
    def _view(self):
        ...
        elif upload_src.filename.split('.')[-1] in ["ofd", "OFD"]:
            datas, pmessage = ofd_zip_to_json(upload_src)
    ...
    def init_app(self):
        self.__app.add_url_rule(
            rule=self._fields["restfulapi"], endpoint='index', view_func=self._view, methods=["POST"])
